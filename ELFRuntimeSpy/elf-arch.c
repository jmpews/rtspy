    ...
    find_symbol_in_lib(dyn_info_t *dyldinfo, elf_rt_input_t input, char *sym_name)
{
    ...
    xdebug(printf("start gnu hash search:\n\tnew_hash: 0x%x(%u)", sym_name, new_hash, new_hash));
    ...
    xdebug(printf("\thash buckets index: 0x%x(%u), first dynsym index: 0x%x(%u)", (new_hash % dyldinfo->nbuckets), (new_hash % dyldinfo->nbuckets), symndx, symndx));
    ...
    xinfo("start bucket search");
    do
    {
        elf_rt_read(input, hash_addr, &h2, sizeof(unsigned int));
        xdebug(printf("\th2: 0x%x(%u)", h2, h2));
        /*1. hash value same */
        if (((h2 ^ new_hash) >> 1) == 0)
        {
...
long find_symbol(elf_rt_t *target, char *sym_name, char *lib_name)
{
    ...
    xdebug(printf("start search \'%s\':", sym_name));
    ...
    while (!sym_addr && linkmap_addr)
    {
        ...
        xdebug(printf("search libaray path: %s", soname));
        ...
        if (sym)
        {
            ...
            xdebug(printf("found \'%s\' at %p", sym_name, sym_addr));
            return sym_addr;
        }
    }
